      ...
      if (absEmojiStart) {
        ...
        const makeSuggestion = function ({ emoji, name, code }) {
          return (
            ...
            >
              <img alt={emoji} class="emoji" draggable="false" loading="lazy" src={`${getoption()}${getoption(1) || twemoji.size}/${code}${getoption(2) || twemoji.ext}`} />
              {name}
            </button>
          );
        };
  ...
  extend(TextEditor.prototype, 'toolbarItems', function (items) {
    items.add(
      ...
      <TextEditorButton onclick={() => this.attrs.composer.editor.insertAtCursor(' :')} icon="far fa-smile">
        {app.translator.trans('flarum-twemoji.forum.composer.emoji_tooltip')}
      </TextEditorButton>
    );
  });
}
